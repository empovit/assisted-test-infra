...
class AgentClusterInstall(BaseCustomResource):
    ...
    def create(
        ...
        **kwargs,
        ...
        metadata.update({"annotations": {"agent-install.openshift.io/install-config-overrides": "{\"networking\": {\"networkType\": \"OVNKubernetes\", \"machineNetwork\": [{\"cidr\": \"1001:db8::/120\"}, {\"cidr\": \"192.168.126.0/24\"}], \"clusterNetwork\": [{\"cidr\": \"fd01::/48\", \"hostPrefix\": 64}, {\"cidr\": \"10.128.0.0/14\", \"hostPrefix\": 23}], \"serviceNetwork\": [\"fd02::/112\", \"172.30.0.0/16\"]}}"}})
        body = {
